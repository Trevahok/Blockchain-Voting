...
class Blockchain():
    ...
    def register_node(self , address, flag):
        ...
        if flag == 1:
            ...
            for node in self.nodes:
                node = "http://" + node
                requests.post(url='http://'+parsed_url.netloc+'/nodes/register', json={
                    'nodes': [node],
                    'flag': 0
                })
    ...
    def triggered_flood_chain(self):
        for node in self.nodes:
            response = requests.get('http://'+node+'/nodes/resolve')

    def trigger_flood_nodes(self,address):
        print('flooding now ')
        for node in self.nodes:
            requests.post(url='http://'+node+'/nodes/register', json={
                'nodes': [address] ,
                'flag': 0
            })
